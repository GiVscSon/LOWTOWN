export function createNavigationBrain({nodes=[],blocked=()=>false}={}) {
  ...
  function choose(start,goal=null){const pool=nodes.filter(safe);if(!pool.length)return null;const recent=new Set(state.recent.slice(-10));let best=null,score=-Infinity;for(const n of pool){if(goal&&n.id===goal.id)continue;const k=key(n.x,n.y),visits=state.cells.get(k)||0;const far=dist(start,n);const novelty=recent.has(k)?0:1;const edge=n.links.length===1?-500:0;const s=Math.min(far,1400)*.45-n.visits*170+novelty*520+edge;if(s>score){score=s;best=n;}}state.destination=best;state.lastReason='EXPLORE';return best;}
  ...
  function status(){return {enabled:state.enabled,cells:state.cells.size,loops:state.loops,deadEnds:state.deadEnds,recoveries:state.recoveries,destination:state.destination,lastReason:state.lastReason};}
