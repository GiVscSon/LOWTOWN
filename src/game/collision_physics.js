...
export function circleContact(a,b,{restitution=.12,friction=.45,damageScale=.002,maxImpulse=140,separation=.82}={}){
  ...
  const move=Math.min(penetration*.5*clamp(separation,0,1),18);
  ...
  const rawImpulse=-(1+clamp(finite(restitution,.12),0,1))*closing/sumInv,impulse=Math.min(Math.max(0,finite(maxImpulse,140)),rawImpulse);
