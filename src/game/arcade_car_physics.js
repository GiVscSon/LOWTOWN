...
export function stepArcadeCar(state,dt,input={},profile={}){
 ...
 const brake=Math.max(40,finite(profile.arcadeBrake,260));
 const drag=Math.max(.05,finite(profile.arcadeDrag,.9));
 ...
 if(throttle>0)state.v+=acceleration*throttle*h;else if(throttle<0)state.v+=reverseAcceleration*throttle*h;else state.v*=Math.max(0,1-drag*h);
 if(brakeInput>0){const amount=brake*brakeInput*h;state.v-=Math.sign(state.v||1)*Math.min(Math.abs(state.v),amount);}
 if(input.handbrake)state.v*=Math.max(0,1-2.8*h);
 state.v=clamp(state.v,-maxSpeed*.28,maxSpeed);
 state.yawRate=Math.abs(state.v)>0.5?(state.v/wheelbase)*Math.tan(state.steerAngle):0;
