...
const scaleFromRatios=(values,fallback=1)=>{const usable=values.filter(v=>Number.isFinite(v)&&v>.05&&v<4);return clamp(robustMean(usable.length?usable:[fallback]),.75,1.25);};
...
export function buildPhysicsErrorProfile(report={}){
  ...
  for(const [id,v] of Object.entries(report.vehicles||{})){
    ...
    vehicles[id]={vehicleId:id,accelerationBias:+clamp(finite(v.meanAccelerationError),-50,50).toFixed(5),steeringBias:+clamp(finite(v.meanSteeringError),-10,10).toFixed(5),accelerationScale:scaleFromRatios([v.driveScale],1),brakingScale:scaleFromRatios([v.brakeScale],1),steeringScale:scaleFromRatios([v.steeringScale],1),dragScale:scaleFromRatios([v.dragScale],1),turnRadiusScale:scaleFromRatios([v.turnRadiusScale],1),speedLimitViolations:Number(v.speedLimitViolations)||0,confidence:Math.min(1,samples/120),samples,ready:samples>=30};
  }
...
export function calibrateSamples(samples=[],{minSamples=30,maxBiasAccel=50,maxBiasSteer=10}={}){
  ...
  for(const [id,list] of Object.entries(groups)){
    ...
    const n=list.length;
    vehicles[id]={vehicleId:id,accelerationBias:+clamp(accel,-maxBiasAccel,maxBiasAccel).toFixed(5),steeringBias:+clamp(steer,-maxBiasSteer,maxBiasSteer).toFixed(5),accelerationScale:scaleFromRatios(list.map(s=>s.driveScale),1),brakingScale:scaleFromRatios(list.map(s=>s.brakeScale),1),steeringScale:scaleFromRatios(list.map(s=>s.steeringScale),1),dragScale:scaleFromRatios(list.map(s=>s.dragScale),1),turnRadiusScale:scaleFromRatios(list.map(s=>s.turnRadiusScale),1),confidence:Math.min(1,n/minSamples),samples:n,ready:n>=minSamples};
  }
  return {version:1,vehicles};
}
...
export function calibrationScalesFromError({accelerationBias=0,steeringBias=0,brakingBias=0,dragBias=0,turnRadiusBias=0,confidence=0}={}){
  return {accelerationScale:clamp(1-accelerationBias/20,.75,1.25),brakingScale:clamp(1-brakingBias/20,.75,1.25),steeringScale:clamp(1-steeringBias/4,.75,1.25),dragScale:clamp(1+dragBias/20,.75,1.25),turnRadiusScale:clamp(1+turnRadiusBias/20,.75,1.25),confidence:clamp(confidence,0,1)};
}
