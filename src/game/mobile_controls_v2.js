...
export function createMobileControlsV2(){
  ...
  function updateStick(e){
    ...
    const x=dx/max,y=dy/max,dead=.16;
    setKey('a',x<-dead);setKey('d',x>dead);setKey('w',y<-dead);setKey('s',y>dead);
  }
  ...
  aiButton.addEventListener('pointerdown',e=>{e.preventDefault();ai=!ai;releaseDrive();keyEvent('keydown','i');keyEvent('keyup','i');aiButton.textContent=ai?'AI: ON':'AI: OFF';aiButton.classList.toggle('pressed',ai)},{passive:false});
  resetButton.addEventListener('pointerdown',e=>{e.preventDefault();releaseDrive();keyEvent('keydown','r');keyEvent('keyup','r')},{passive:false});
  handbrakeButton.addEventListener('pointerdown',e=>{e.preventDefault();handbrakeButton.setPointerCapture?.(e.pointerId);setKey(' ',true);handbrakeButton.classList.add('pressed')},{passive:false});
