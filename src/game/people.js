...
export function createPeopleSystem({ nodes, blocked, seed = 4242 }) {
  ...
  for (let i = 0; i < Math.min(48, nodes.length); i++) { const n = pick(); people.push({ x:n.x+(rand()-.5)*70,y:n.y+(rand()-.5)*70,a:rand()*Math.PI*2,v:22+rand()*20,state:'walk',timer:rand()*4,tone:rand(),target:pick(),panic:0 }); }
  function update(dt, player, danger = 0) {
    for (const p of people) {
      ...
      if (p.timer<=0 || Math.hypot(p.target.x-p.x,p.target.y-p.y)<24) { p.target=pick();p.timer=2+rand()*5;p.state=rand()<.18?'idle':'walk'; }
      if(p.state==='idle'&&p.panic<.2)continue;
      let desired=Math.atan2(p.target.y-p.y,p.target.x-p.x); if(p.panic>.2&&pd<260)desired=Math.atan2(p.y-player.y,p.x-player.x);
  ...
  function draw(ctx, iso, player) {
    for(const p of people){const q=iso(p.x,p.y),near=player&&Math.hypot(player.x-p.x,player.y-p.y)<260,image=sprites[p.panic>.2?'runner':'civilian'];ctx.save();ctx.translate(q.x,q.y);ctx.rotate(-p.a);if(image.complete)ctx.drawImage(image,-12,-16,24,18);else{ctx.fillStyle='#596068';ctx.fillRect(-3,-7,6,11);}if(p.panic>.2&&near){ctx.fillStyle='#e8b84a';ctx.font='9px monospace';ctx.textAlign='center';ctx.fillText('RUN!',0,-20)}ctx.restore();}
  }
