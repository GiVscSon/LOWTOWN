...
export function createPeopleSystem({nodes,blocked,seed=4242}){
 ...
 const sidewalkPoint=n=>{for(let attempt=0;attempt<8;attempt++){const next=n?.links?.[(rand()*Math.max(1,n.links?.length||1))|0]||n;if(!next)continue;const dx=next.x-n.x,dy=next.y-n.y,len=Math.hypot(dx,dy)||1,side=rand()<.5?-1:1,p={x:n.x-dy/len*58*side,y:n.y+dx/len*58*side};if(!blocked(p.x,p.y))return p;}return{x:n.x,y:n.y};};
 const freeSidewalkPoint=()=>{for(let i=0;i<activeNodes.length*2;i++){const p=sidewalkPoint(pick());if(!people.some(q=>Math.hypot(q.x-p.x,q.y-p.y)<28))return p;}return sidewalkPoint(pick());};
 const chooseTarget=p=>{const target=freeSidewalkPoint();p.target=target;p.timer=2+rand()*5;p.state=rand()<.12?'idle':'walk';p.waypoint=0;};
 for(let i=0;i<Math.min(28,activeNodes.length);i++){const q=freeSidewalkPoint();people.push({x:q.x,y:q.y,a:0,v:18+rand()*10,state:'walk',timer:rand()*3,tone:rand(),target:freeSidewalkPoint(),route:[],waypoint:0,panic:0,stuck:0});}
