...
export function buildLayeredRoadTopology() {
  // Runtime road topology is an adapter over the same independent geometry
  // authority used by collision/navigation. There is no procedural grid.
  const nodes = buildRoadNetwork();
  for (const node of nodes) {
    node.level = ROAD_LEVELS.STREET;
    node.z = ROAD_LEVEL_Z[ROAD_LEVELS.STREET];
  }
