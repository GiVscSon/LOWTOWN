...
export function createMissionSystem(world) {
  ...
  function syncMissionAI(car, p) {
    ...
    if (key !== aiRouteKey) {
      if (typeof ai.setMissionGoal === 'function') { if (!ai.setMissionGoal(p)) return; }
      else { const route = vehicleRoute({ x: car.x, y: car.y }, p).filter(Boolean); if (!route.length) return; ai.state.goal = route[route.length - 1]; ai.state.route = route; ai.state.node = 0; }
      ai.state.routeTimer = 0; ai.state.routeLockRemaining = MISSION_ROUTE_LOCK; ai.state.routeLocked = true; ai.state.routeLockReason = 'MISSION'; ai.state.mode = ai.state.state = 'MISSION'; ai.state.replans = (ai.state.replans || 0) + 1; aiRouteKey = key;
    } else {
      ai.state.routeTimer = 0; ai.state.mode = ai.state.state === 'IDLE' ? 'MISSION' : ai.state.state; ai.state.routeLocked = true; ai.state.routeLockReason = 'MISSION'; if (Number.isFinite(ai.state.routeLockRemaining)) ai.state.routeLockRemaining = Math.max(0, ai.state.routeLockRemaining - 1 / 60);
    }
  }
  function emergencyReplan(car) {
    const ai = globalThis.__LOWTOWN_AI;
    if (!ai?.state?.enabled || !car || ai.state.mode !== 'MISSION' || !(ai.state.stuckTime > EMERGENCY_STUCK) || typeof ai.replan !== 'function') return;
    if (ai.replan(car)) { ai.state.stuckTime = 0; ai.state.routeTimer = 0; ai.state.routeLocked = true; ai.state.routeLockRemaining = MISSION_ROUTE_LOCK; ai.state.routeLockReason = 'MISSION_EMERGENCY'; ai.state.mode = ai.state.state = 'MISSION'; ai.state.recoveries = (ai.state.recoveries || 0) + 1; }
  }
  function update(car) {
    if (complete) return false;
    const p = target(); syncMissionAI(car, p); emergencyReplan(car);
    if (Math.hypot(car.x - p.x, car.y - p.y) < (p.radius || 55)) {
      started = true; stage++; aiRouteKey = null;
      if (stage >= current().route.length) { complete = true; completedJobs = Math.max(completedJobs, active + 1); const ai = globalThis.__LOWTOWN_AI; if (ai?.clearMissionLock) ai.clearMissionLock(); return true; }
      const ai = globalThis.__LOWTOWN_AI; if (ai?.state?.enabled && ai?.setMissionGoal) ai.setMissionGoal(target()); aiRouteKey = `${active}:${stage}`;
    }
    return false;
  }
  function next() {
    if (!complete) return false;
    const ai = globalThis.__LOWTOWN_AI; if (ai?.clearMissionLock) ai.clearMissionLock();
    active = Math.min(active + 1, templates.length - 1); stage = 0; complete = false; started = false; aiRouteKey = null; return true;
  }
