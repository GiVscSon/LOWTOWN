...
export function nearestRoad(point,{use=USE.BOTH,zone=null}={}){
  const candidates=CITY_ROADS.filter(r=>!zone||r.zone===zone);
  let best=null;
  for(const road of candidates)for(let i=0;i<road.points.length;i++){
    const p=copyPoint(road.points[i]),d=distance(point,p);
    if(!best||d<best.distance)best={road,point:p,index:i,distance:d,use};
  }
  return best;
}
...
export function buildCityGraph(){
  const nodes=[];
  for(const road of CITY_ROADS)for(let i=0;i<road.points.length;i++)nodes.push({id:`${road.id}:${i}`,roadId:road.id,index:i,x:road.points[i][0],y:road.points[i][1],links:[]});
  const byId=new Map(nodes.map(n=>[n.id,n]));
  for(const road of CITY_ROADS)for(let i=0;i<road.points.length-1;i++){
    const a=byId.get(`${road.id}:${i}`),b=byId.get(`${road.id}:${i+1}`);
    a.links.push(b);if(!road.oneWay)b.links.push(a);
  }
  for(const n of nodes)for(const road of CITY_ROADS)if(road.id!==n.roadId)for(let i=0;i<road.points.length;i++){
    const p=road.points[i];if(Math.hypot(p[0]-n.x,p[1]-n.y)<90){const other=byId.get(`${road.id}:${i}`);if(other&&!n.links.includes(other))n.links.push(other);}
  }
  return nodes;
}
...
export function shortestRoute(start,goal){
  ...
  const path=[];for(let id=g.id;id!==undefined;id=prev.get(id)){const n=nodes.find(v=>v.id===id);if(n)path.unshift(n);if(id===s.id)break;}return path;
}
