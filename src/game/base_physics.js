...
export function getBasePhysics(type=TRANSPORT_TYPES.CAR){
  return BASE_PHYSICS[type] || BASE_PHYSICS[TRANSPORT_TYPES.CAR];
}

export function deriveVehicleForces(base,vehicle){
  const mass=Math.max(1,Number(vehicle.mass)||1);
  const power=Math.max(0,Number(vehicle.power)||0);
  const brakes=Math.max(0,Number(vehicle.brakes)||0);
  const engineForce=base.engineForce*power;
  const reverseForce=base.reverseForce*power;
  const brakeForce=base.brakeForce*brakes;
