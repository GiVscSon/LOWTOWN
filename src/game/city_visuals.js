...
function buildingDetails(ctx, iso, b, t) {
  ...
  ctx.beginPath();
  ctx.moveTo(base[1].x, base[1].y); ctx.lineTo(base[2].x, base[2].y); ctx.lineTo(roof[2].x, roof[2].y); ctx.lineTo(roof[1].x, roof[1].y); ctx.closePath(); ctx.fill();
  ctx.fillStyle = seed > 0.5 ? '#232629' : '#1e2124';
  ctx.beginPath();
  ctx.moveTo(base[3].x, base[3].y); ctx.lineTo(base[2].x, base[2].y); ctx.lineTo(roof[2].x, roof[2].y); ctx.lineTo(roof[3].x, roof[3].y); ctx.closePath(); ctx.fill();
  ...
  ctx.strokeStyle = 'rgba(224,154,62,0.18)'; ctx.stroke();
  if (w > 100 && h > 85) {
    const rx = roof[0].x * 0.4 + roof[2].x * 0.6;
    const ry = roof[0].y * 0.4 + roof[2].y * 0.6;
    ctx.fillStyle = '#181a1d'; ctx.fillRect(rx - 8, ry - 5, 16, 9);
    ctx.strokeStyle = 'rgba(154,160,168,0.3)'; ctx.strokeRect(rx - 8, ry - 5, 16, 9);
  }
  if (seed > 0.72 && w > 90) {
    const sx = (roof[3].x + roof[2].x) * 0.5;
    const sy = (roof[3].y + roof[2].y) * 0.5 - 6;
    ctx.fillStyle = '#0b0d10'; ctx.fillRect(sx - 18, sy - 8, 36, 12);
    ctx.strokeStyle = 'rgba(224,154,62,0.5)'; ctx.strokeRect(sx - 18, sy - 8, 36, 12);
    ctx.fillStyle = 'rgba(224,154,62,0.85)'; ctx.font = 'bold 6px monospace'; ctx.textAlign = 'center';
...
function junctionPlates(ctx, iso) {
  ctx.save();
  for (const junction of CITY_JUNCTIONS.junctions) {
    const p = iso(junction.x, junction.y);
    ctx.fillStyle = '#2f3236'; ctx.beginPath(); ctx.ellipse(p.x, p.y, 28, 16, 0, 0, Math.PI * 2); ctx.fill();
...
function streetAtmosphere(ctx, iso, lamps, t) {
  ...
  for (let i = 0; i < lamps.length; i++) {
    ...
    g.addColorStop(0, 'rgba(255,219,145,.28)'); g.addColorStop(0.35, 'rgba(224,154,62,.12)'); g.addColorStop(1, 'rgba(224,154,62,0)');
...
function roadLabels(ctx, iso) {
  ...
  for (const road of CITY_ROADS) {
    const mid = Math.floor((road.points.length - 1) / 2), p = roadPoint(road.id, mid, sidewalkOffset(road));
    if (!p) continue;
    const q = iso(p.x, p.y); ctx.translate(q.x, q.y - 8); ctx.rotate(-p.heading * .55);
    ctx.font = road.class === 'ARTERIAL' ? 'bold 8px monospace' : '7px monospace'; ctx.fillStyle = 'rgba(224,154,62,.72)';
    ctx.shadowColor = 'rgba(0,0,0,.9)'; ctx.shadowBlur = 3; ctx.fillText(road.name.toUpperCase(), 0, 0); ctx.setTransform(1, 0, 0, 1, 0, 0);
  }
  ctx.restore();
}
...
function destinationSigns(ctx, iso) {
  for (const d of CITY_DESTINATIONS) {
    const road = CITY_ROADS.find(r => r.id === d.roadId);
    const p = road ? roadPoint(d.roadId, d.index, sidewalkOffset(road)) : null;
    if (!p) continue;
    const q = iso(p.x, p.y); ctx.fillStyle = 'rgba(7,9,11,.86)'; ctx.fillRect(q.x - 31, q.y - 29, 62, 12); ctx.strokeStyle = 'rgba(224,154,62,.22)'; ctx.strokeRect(q.x - 31, q.y - 29, 62, 12); ctx.fillStyle = 'rgba(190,194,196,.78)'; ctx.font = '6px monospace'; ctx.textAlign = 'center'; ctx.fillText(d.name.toUpperCase(), q.x, q.y - 20);
  }
}
