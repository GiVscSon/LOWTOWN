...
const roadNodes=buildRoadNetwork({isLand,blocked,limit:ROAD_LIMIT,grid:ROAD_GRID});
const roadLines=roadSegments(roadNodes);
function nearestRoadPoint(x,y){let best=null;for(const [a,b] of roadLines){const dx=b.x-a.x,dy=b.y-a.y,len2=dx*dx+dy*dy||1;const t=Math.max(0,Math.min(1,((x-a.x)*dx+(y-a.y)*dy)/len2));const px=a.x+dx*t,py=a.y+dy*t,d=Math.hypot(x-px,y-py);if(!best||d<best.distance)best={x:px,y:py,distance:d,heading:Math.atan2(dy,dx)};}return best;}
...
function playerCar(){const p=iso(S.car.x,S.car.y);const f=iso(S.car.x+Math.cos(S.car.a)*10,S.car.y+Math.sin(S.car.a)*10);const ang=Math.atan2(f.y-p.y,f.x-p.x);ctx.save();ctx.translate(p.x,p.y);ctx.rotate(ang);ctx.fillStyle='rgba(0,0,0,.55)';ctx.beginPath();ctx.ellipse(0,9,34,7,0,0,Math.PI*2);ctx.fill();ctx.fillStyle='#252a2e';ctx.beginPath();ctx.moveTo(-28,-10);ctx.lineTo(25,-10);ctx.lineTo(35,6);ctx.lineTo(25,11);ctx.lineTo(-28,11);ctx.lineTo(-36,4);ctx.closePath();ctx.fill();ctx.fillStyle='#e8b84a';ctx.strokeStyle='#171a1e';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(-26,-8);ctx.lineTo(23,-8);ctx.lineTo(31,5);ctx.lineTo(22,8);ctx.lineTo(-25,8);ctx.lineTo(-31,3);ctx.closePath();ctx.fill();ctx.stroke();ctx.fillStyle='#1b2228';ctx.beginPath();ctx.moveTo(-13,-5);ctx.lineTo(10,-5);ctx.lineTo(20,4);ctx.lineTo(-18,4);ctx.closePath();ctx.fill();ctx.fillStyle='#d4523a';ctx.fillRect(-27,3,7,3);ctx.fillStyle='#fff1a7';ctx.fillRect(22,2,6,4);ctx.fillStyle='#0a0b0d';ctx.fillRect(-20,-12,11,4);ctx.fillRect(11,-12,11,4);ctx.restore();}
function speed(){return Math.hypot(S.car.vx||0,S.car.vy||0);}
function keepPlayerOnRoad(before){const near=nearestRoadPoint(S.car.x,S.car.y);if(!near||near.distance>ROAD_WIDTH*.62){S.car.x=before.x;S.car.y=before.y;S.car.a=before.a;S.car.v*=.2;S.car.vx*=.2;S.car.vy*=.2;S.car.yawRate*=.2;S.stuck++;return false;}S.stuck=0;return true;}
...
function trafficCollisions(){const now=performance.now();for(const n of traffic.cars){const dx=n.x-S.car.x,dy=n.y-S.car.y,d=Math.hypot(dx,dy);if(d<34&&now-(n.hitAt||0)>550){const nx=dx/(d||1),ny=dy/(d||1),impact=Math.max(20,speed()-n.v);S.car.vx-=nx*impact*.12;S.car.vy-=ny*impact*.12;n.v=Math.max(0,n.v-impact*.12);n.x-=nx*(34-d)*.5;n.y-=ny*(34-d)*.5;n.hitAt=now;S.trafficHits++;S.damage+=Math.min(3,impact/90);toast.textContent='TRAFFIC HIT';}}}
...
function reset(){const near=nearestRoadPoint(S.car.x,S.car.y)||start;S.car.x=near.x;S.car.y=near.y;S.car.z=0;S.car.a=near.heading;S.car.v=0;S.car.vx=0;S.car.vy=0;S.car.vz=0;S.car.yawRate=0;S.car.distance=0;S.car.age=0;playerTransport.resetActuators();playerTransport.setVehicle('sedan');S.cam.x=S.car.x;S.cam.y=S.car.y;S.done=false;S.damage=0;S.collisions=0;S.trafficHits=0;S.stuck=0;S.maxSpeed=0;S.distance=0;S.aiDebug={frames:0,elapsed:0,movedDistance:0,initialX:S.car.x,initialY:S.car.y,last:null};if(S.aiActive)ai.start(S.car);else ai.state.enabled=false;toast.classList.remove('visible','hot');refreshMission();}
...
function draw(){roads();for(const b of buildings)building(...b);for(const l of lamps)lamp(...l);cityVisuals.draw(ctx,iso,buildings,lamps,S.t);traffic.draw(ctx,iso);people.draw(ctx,iso,S.car);transport.draw(ctx,iso,S.t*1000);target();playerCar();}
