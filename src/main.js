...
import { ROAD_GRID, ROAD_LIMIT, ROAD_WIDTH } from './game/road_authority.js';
...
app.innerHTML=`<main class="shell"><header class="hud"><div class="brand"><span>LOW</span>TOWN <b>// NIGHT SHIFT</b></div><div class="status"><i></i> FREE ROAM <strong id="speed">000</strong> KM/H <b id="cash">CASH $0000</b></div></header><section class="game-wrap"><canvas id="game"></canvas><div class="mission"><small id="job-id">JOB 01</small><strong id="job-title">SHAKE THE NIGHT</strong><span id="job-text">Drive to the amber marker.</span></div><div class="hint">WASD / ARROWS · SPACE HANDBRAKE · I AI DRIVE · R RESET · N NEXT JOB</div><div class="toast" id="toast">ENGINE READY</div></section></main>`;
...
const SAVE_KEY='lowtown.progress.v1';
const KMH_PER_WORLD_SPEED=.19;
const loadProgress=()=>{try{const p=JSON.parse(localStorage.getItem(SAVE_KEY)||'{}');return{money:Number.isFinite(Number(p.money))?Math.max(0,Math.floor(Number(p.money))):0,completedJobs:Number.isFinite(Number(p.completedJobs))?Math.max(0,Math.floor(Number(p.completedJobs))):0};}catch{return{money:0,completedJobs:0};}};
const saveProgress=()=>{try{localStorage.setItem(SAVE_KEY,JSON.stringify({money:S.money,completedJobs:S.completedJobs}));}catch{}};
...
const start=nearestRoadPoint(-320,0,ROAD_LEVELS.STREET)||{x:-320,y:0,heading:0,level:ROAD_LEVELS.STREET};
...
const S={car:playerTransport.state,cam:{x:start.x,y:start.y},target:WORLD.mission||{x:start.x,y:start.y},t:0,done:false,damage:0,collisions:0,trafficHits:0,stuck:0,maxSpeed:0,distance:0,missionReward:0,money:saved.money,completedJobs:saved.completedJobs,aiActive:testMode,aiDebug:{frames:0,elapsed:0,movedDistance:0,initialX:start.x,initialY:start.y,last:null}};
S.car.roadLevel=ROAD_LEVELS.STREET;
missions.restore(S.completedJobs);
S.completedJobs=missions.state().completedJobs;
...
function target(){const p=iso(S.target.x,S.target.y,ROAD_LEVEL_Z[S.car.roadLevel]??ROAD_LEVEL_Z[1]),r=20+Math.sin(S.t*5)*5;ctx.save();ctx.strokeStyle='#e09a3e';ctx.lineWidth=4;ctx.beginPath();ctx.arc(p.x,p.y,r,0,Math.PI*2);ctx.stroke();ctx.fillStyle='rgba(224,154,62,.2)';ctx.fill();ctx.fillStyle='#e09a3e';ctx.font='bold 11px monospace';ctx.textAlign='center';ctx.fillText('DROP',p.x,p.y-31);ctx.beginPath();ctx.moveTo(p.x,p.y-16);ctx.lineTo(p.x-7,p.y-25);ctx.lineTo(p.x+7,p.y-25);ctx.closePath();ctx.fill();ctx.restore();}
...
function refreshMission(){const m=missions.state();if(!m)return;const idx=missions.templates.findIndex(t=>t.id===m.id);S.target=m.target||S.target;jobId.textContent=`JOB ${String(idx+1).padStart(2,'0')}`;jobTitle.textContent=m.title||'FREE ROAM';jobText.textContent=m.complete?'JOB COMPLETE // PRESS N FOR NEXT':m.text||'Drive through LOWTOWN.';cashEl.textContent=`CASH $${String(Math.max(0,Math.floor(S.money))).padStart(4,'0')}`;}
...
function update(dt){if(keys.has('n')){if(missions.next()){S.done=false;refreshMission();}keys.delete('n');}if(keys.has('r')){reset();keys.delete('r');}if(keys.has('i')){toggleAI();keys.delete('i');}if(S.aiActive)updateAI(dt);else{const u=keys.has('w')||keys.has('arrowup'),d=keys.has('s')||keys.has('arrowdown'),l=keys.has('a')||keys.has('arrowleft'),r=keys.has('d')||keys.has('arrowright');drive({throttle:u?1:d?-1:0,brake:d?1:0,steer:(r?1:0)-(l?1:0),handbrake:keys.has(' ')},dt);}events.update(dt,S.car);traffic.update(dt,S.car,events.state());people.update(dt,S.car,0);transport.update(dt);trafficCollisions();const missionResult=missions.update(S.car);const missionState=missions.state();S.target=missionState.target||S.target;S.completedJobs=missionState.completedJobs;if(!S.done&&missionResult){S.done=true;const reward=missions.state().reward||0;S.missionReward+=reward;S.money+=reward;saveProgress();toast.textContent=`JOB COMPLETE // +$${reward}`;toast.classList.add('hot');}const v=Number.isFinite(Number(S.car.telemetry?.velocity))?Number(S.car.telemetry.velocity):speed();S.maxSpeed=Math.max(S.maxSpeed,v);S.cam.x+=(S.car.x-S.cam.x)*Math.min(1,dt*7);S.cam.y+=(S.car.y-S.cam.y)*Math.min(1,dt*7);const kmh=Math.max(0,Math.round(v*KMH_PER_WORLD_SPEED));speedEl.textContent=String(kmh).padStart(3,'0');cashEl.textContent=`CASH $${String(Math.max(0,Math.floor(S.money))).padStart(4,'0')}`;}
