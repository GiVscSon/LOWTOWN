...
function keepPlayerOnRoad(before){updateRoadLevel();const level=S.car.roadLevel??ROAD_LEVELS.STREET;const near=nearestLayeredRoadPoint(S.car.x,S.car.y,roadNodes,level);const blockedNow=vehicleWorldBlocked(S.car.x,S.car.y,S.car.a,roadLines,{length:56,width:28})||!near||near.distance>corridorForHit(near);if(blockedNow){const fallback=nearestLayeredRoadPoint(before.x,before.y,roadNodes,level)||nearestAnyRoadPoint(before.x,before.y,roadNodes);if(fallback){S.car.x=fallback.x;S.car.y=fallback.y;S.car.roadLevel=fallback.level??level;S.car.a=fallback.heading;}else{S.car.x=before.x;S.car.y=before.y;S.car.a=before.a;}S.car.v*=.2;S.car.vx*=.2;S.car.vy*=.2;S.car.yawRate*=.2;S.stuck++;return false;}S.stuck=0;return true;}
