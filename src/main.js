...
import { ROAD_EDGE_TOLERANCE } from './game/road_authority.js';
...
import { collisionHalfWidth, bridgePoints } from './game/road_geometry.js';
import { roadById } from './game/city_semantics.js';
...
const roadNodes=buildLayeredRoadTopology();
const roadLines=layeredRoadSegments(roadNodes);
const streetNodes=roadNodes.filter(n=>n.level===ROAD_LEVELS.STREET);
function corridorForHit(hit){const road=roadById(hit?.a?.roadId||hit?.b?.roadId);return road?collisionHalfWidth(road)+ROAD_EDGE_TOLERANCE:0;}
function onRoadCorridor(x,y,level=ROAD_LEVELS.STREET){const hit=nearestLayeredRoadPoint(x,y,roadNodes,level)||nearestAnyRoadPoint(x,y,roadNodes);return!!hit&&hit.distance<=corridorForHit(hit);}
...
const traffic=createTrafficSystem({nodes:streetNodes,blocked,roadLines});
...
function terrain(){ctx.fillStyle='#0b171b';ctx.fillRect(0,0,canvas.clientWidth,canvas.clientHeight);for(const island of ISLANDS){poly(ellipseWorld(island),island.colors.land,'#0b0d0f');const inner={...island,rx:island.rx-42,ry:island.ry-42};poly(ellipseWorld(inner),island.biome==='FOREST_HIGHLAND'?'#263329':island.biome==='INDUSTRIAL_COAST'?'#292c2d':'#24272a');}for(const b of BRIDGES){const pts=bridgePoints(b);if(pts.length<2)continue;ctx.strokeStyle='#4b4035';ctx.lineWidth=b.width||40;ctx.beginPath();pts.forEach(([x,y],i)=>{const p=iso(x,y,ROAD_LEVEL_Z[ROAD_LEVELS.STREET]);i?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y);});ctx.stroke();ctx.strokeStyle='#d6a15a';ctx.lineWidth=2;ctx.setLineDash([10,10]);ctx.beginPath();pts.forEach(([x,y],i)=>{const p=iso(x,y,ROAD_LEVEL_Z[ROAD_LEVELS.STREET]);i?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y);});ctx.stroke();ctx.setLineDash([]);}}
...
function updateRoadLevel(){const current=S.car.roadLevel??ROAD_LEVELS.STREET;const hit=nearestLayeredRoadPoint(S.car.x,S.car.y,roadNodes,current);if(hit&&hit.distance<=corridorForHit(hit))return;const any=nearestAnyRoadPoint(S.car.x,S.car.y,roadNodes);if(any&&any.distance<=corridorForHit(any))S.car.roadLevel=any.level;}
function keepPlayerOnRoad(){
  // Transport controller is now the single runtime authority for vehicle containment.
  // Do not snap position/heading back to a nearest road point here: that legacy
  // correction caused heading discontinuities and sideways-looking recovery motion.
  updateRoadLevel();
  S.stuck=0;
  return true;
}
function drive(input,dt){const before={x:S.car.x,y:S.car.y,a:S.car.a};playerTransport.step(dt,input);keepPlayerOnRoad();S.distance+=speed()*dt;}
